...
ForwardRequest
	...
	•	padding_to: u32 (0 = none; otherwise padded to fixed MAX_BATCH for CUDA Graphs)
...
3.1 Tasks/Threads at a Glance
	•	Tokio runtime (current_thread) hosts:
...
Day 1
	...
	3.	Rust skeleton: start Tokio (current_thread), wire tonic client over UDS; implement feeder with GLOBAL_REQ_MPSC, COMPLETION_MPSC, credits; stub router.
...
loop {
  while tokens > 0 && have_work() {
     push_request(); tokens -= 1;
  }
  let ev = await next_result();
  tokens += 1; continue_search_with(ev);   // on Rayon
}
...
11) Common Failure Modes & Quick Fixes
	•	GPU idle gaps every few ms → FLUSH_US too high or W=1. Reduce FLUSH_US to ~200 µs; set W=2.
	•	Batch sizes oscillate (64 → 1024) → enable target/max split and time‑boxed flush; ensure feeder never awaits RPC completion.
	•	One game hogs the world → lower per_game_inflight; use round‑robin drain.
	•	CPU spikes stall feeder → ensure all heavy continuations run on Rayon; keep feeder on Tokio’s current_thread executor.
	•	gRPC overhead visible → confirm UDS, large messages (flat bytes), and single RPC per batch.
	•	CUDA Graph flaky → disable; the rest of the pipeline is already good enough.

⸻

12) What to Defer (don’t gold‑plate)
	•	Multi‑GPU sharding, distributed queues, elaborate schedulers
	•	Sophisticated prioritisation; real‑time SLAs
	•	Full observability stack; a 1‑line/sec summary is enough now

⸻

13) Final Checklist (ship‑it)
	•	Feeder owns batching, credits, and never blocks on RPC
	•	≥2 in‑flight batches end‑to‑end
	•	Bounded global queue; per‑game token caps
	•	Python server with preallocation + inference mode + autocast
	•	Inputs as u8 exponents; outputs [B,4] floats
	•	Metrics show steady batch sizes and high device activity

⸻

Appendix A — Minimal .proto sketch

syntax = "proto3";
package inference;

service Inference {
  rpc ForwardBatch(ForwardRequest) returns (ForwardResponse);
}

message ForwardRequest {
  uint64 batch_id = 1;
  repeated uint64 request_ids = 2;
  bytes boards = 3;           // [B,16] u8
  enum DType { FP16=0; BF16=1; FP32=2; }
  DType dtype_hint = 4;
  uint32 padding_to = 5;      // 0 = none
}

message ForwardResponse {
  uint64 batch_id = 1;
  repeated uint64 request_ids = 2;
  bytes evs = 3;              // [B,4] floats
  string aux_json = 4;
}

Appendix B — Default Parameters (start here)
	•	CUDA: TARGET_BATCH=512, MAX_BATCH=1024, FLUSH_US=250, W=2, C_game=16
	•	MLX:  TARGET_BATCH=256, MAX_BATCH=512,  FLUSH_US=300, W=2, C_game=16

If you can only change one thing to raise utilisation: increase steady batch size and ensure W≥2.