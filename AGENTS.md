...
Sync the environment with `uv sync`; the project pins dependencies through `pyproject.toml` and `uv.lock`. Launch training with `uv run python main.py --config config/config.example.toml` (set `--device cpu|cuda` as needed). Validate gameplay using `uv run bin/play_2048.py --init ./inits/v1_50m --seed 123`. Benchmark client/server inference via `uv run python benchmarks/bench_client_server.py --init inits/v1_pretrained_50m --uds /tmp/2048_infer.sock --device cuda --compile-mode default --config config/inference/top-score.toml --release`.
