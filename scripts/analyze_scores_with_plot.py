...
def create_histogram_plot(scores, output_file="score_distribution.png"):
    """Create a simple histogram using only built-in modules."""
    try:
        # Simple ASCII histogram
        print("\nCreating ASCII histogram...")
        ...
        for i, count in enumerate(bins):
            ...
            print(f"{bin_start:6.0f} - {bin_end:6.0f}: {'█' * bar_length} ({count})")
...
def main():
    ...
    parser.add_argument('--ascii-only', action='store_true', help='Only output ASCII histogram, no matplotlib')
    ...
    create_histogram_plot(all_scores)
